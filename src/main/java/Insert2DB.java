...
public class Insert2DB {
    ...
    public static void main(String[] args) {
        ...
        DBConnection con;
        File listFile = new File("F:\\ProjectJava\\MY tries\\DownloadExceltoDB\\loadfile");
        File exportFiles[] = listFile.listFiles();
        String[] names = new String[exportFiles.length];

        for (int j = 0; j < names.length; j++) {
            names[j] = exportFiles[j].getName();
            if(names[j].matches("AKT OSAGO(.*)") && names[j].matches("(.*).xlsx")) {
                System.out.println("Считываем файл" + ' ' + names[j]);


                try {
                    con = new DBConnection();
                    FileInputStream input = new FileInputStream(listFile + "\\" + names[j]);
